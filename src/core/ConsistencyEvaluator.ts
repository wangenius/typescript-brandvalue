...
export class BrandConsistencyEvaluator {
   ...
   * 评估品牌屋一致性
   */
  async evaluateBrandHouse(metrics: BrandConsistencyMetrics, analysisReport: string): Promise<ConsistencyResult> {
    // 1. 计算总分
...
`;
    ...
    try {
      ...
      return {
        ...
        weaknesses: result.weaknesses || ["需要加强视觉一致性", "语言风格有待统一"],
        opportunities: result.opportunities || ["可以进一步优化品牌表达", "有机会提升用户体验"],
