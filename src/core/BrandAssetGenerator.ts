...
export class BrandAssetGenerator {
   ...
   * 生成基本品牌信息
   */
  private async generateBasicBrandInfo(content: string): Promise<any> {
...
只返回JSON, 不要其他文字。`;
   ...
   * 生成品牌定位
   */
  private async generateBrandPositioning(
    content: string,
    brandName: string
  ): Promise<any> {
...
只返回JSON，不要其他文字。`;
   ...
   * 生成品牌表达
   */
  private async generateBrandExpression(
    content: string,
    brandName: string
  ): Promise<any> {
...
颜色值请使用标准十六进制格式。只返回JSON，不要其他文字。`;
   ...
   * 生成用户画像
   */
  private async generateUserPersonas(
    content: string,
    brandName: string
  ): Promise<any> {
...
请生成1-3个不同的用户画像。只返回JSON，不要其他文字。`;
   ...
   * 验证并修复缺失字段
   */
  private async validateAndFixMissingFields(
    brandAsset: any,
   ...
   * 生成缺失字段的值
   */
  private async generateMissingField(
    fieldPath: string,
    content: string,
    brandName: string
  ): Promise<any> {
...
请根据字段路径返回合适的值，格式为JSON。只返回该字段的值，不要包装在其他结构中。`;
   ...
   * 从文本中提取品牌数据
   */
  private extractBrandDataFromText(text: string): any {
    try {
      // 尝试提取JSON
   ...
   * 获取字段的默认值
   */
  private getDefaultValueForField(fieldPath: string): any {
